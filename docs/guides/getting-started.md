...
Without optional deep-evaluation tools, `ai-doc check` still runs built-in deterministic
rules. Those rules inspect Markdown structure, instruction clarity, repeated content,
links, approximate token counts, configured budgets, and context-cost signals. Promptfoo
and DeepEval add semantic evaluation on top of those static rules; they do not replace
the default checks.
...
See [Packaging](../operations/packaging.md#source-checkout-mode) for the detailed source-checkout
contract.
...
This builds the core executable for static checks and normal CLI use. If you want the
executable to collect optional Python integrations such as DeepEval or the Promptfoo
Python wrapper, use the build options documented in [Packaging](../operations/packaging.md#standalone-executable).
...
[Packaging](../operations/packaging.md) for build details and smoke tests.
...
If your repository has modules with different documentation layouts, you can either use
wildcards in the root config or add nested `.ai-doc.yaml` files inside those modules.
Nested configs are scoped to their own directory and are merged during a normal root run.
See [Nested Module Configs](configuration.md#nested-module-configs).
...
Static mode uses these built-in rule groups:

- Structure rules check Markdown shape, including titles, heading order, and document
  organization.
...
For operational recovery and CI guidance, see [Runbook](../operations/runbook.md). For
extension examples, see [Extension API](extensions.md).
