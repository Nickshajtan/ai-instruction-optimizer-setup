...
def test_promptfoo_config_uses_named_contains_assertions() -> None:
    ...
    snapshot = DocumentationSnapshot.model_validate(
        {
            "root": ".",
            "documents": [
                {
    ...
    config = _promptfoo_config(snapshot, None, suite)
...
def test_promptfoo_normalize_accepts_success_and_pass_keys() -> None:
    suite = EvaluationSuite.model_validate(
        {"scenarios": [{"id": "one", "task": ""}, {"id": "two", "task": ""}]}
    )

    result = _normalize(
        {"results": [{"success": True}, {"pass": False, "reason": "missing route"}]},
        suite,
    )
...
def test_deepeval_evaluator_uses_loaded_symbols(monkeypatch) -> None:
    ...
    result = DeepEvalEvaluator().evaluate(
        DocumentationSnapshot.model_validate({"root": ".", "documents": []}),
        None,
        suite,
    )
