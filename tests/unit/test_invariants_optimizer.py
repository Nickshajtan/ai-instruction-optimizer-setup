...
from ai_doc.optimizer.generator import ConservativeCandidateGenerator, GenerationStrategyName, StrategyCandidateGenerator
from ai_doc.optimizer.invariants import Invariant, InvariantSemanticStatus, extract_invariants, verify_invariants, verify_invariants_with_semantics
...
class FakeGenerationStrategy:
    ...
    def generate(self, snapshot: DocumentationSnapshot, invariants: list[Invariant]) -> tuple[CandidateProposal, dict[str, str]]:
        ...
        return CandidateProposal(operations=[ProposalOperation(type="retain", reason="fake strategy", expected_clarity_effect="none", expected_finops_effect="none", risk="low")]), {"AGENTS.md": "# Fake\n"}


def test_strategy_candidate_generator_uses_registered_strategy(tmp_path: Path) -> None:
    (tmp_path / "AGENTS.md").write_text("# Rules\n\nUse best practices.\n", encoding="utf-8")
    snapshot = discover_markdown(tmp_path, DEFAULT_CONFIG, ApproximateTokenCounter())
    proposal, rendered = StrategyCandidateGenerator(strategies={GenerationStrategyName.CLARITY: FakeGenerationStrategy()}).generate(snapshot, [], GenerationStrategyName.CLARITY)
...
def test_semantic_generator_receives_search_state(tmp_path: Path) -> None:
    ...
    class FakeSemanticGenerator:
        def generate(self, snapshot, invariants, strategy, previous_summaries, explored_transformations, feedback, memory):
            captured.update({"strategy": strategy, "summaries": previous_summaries, "explored": explored_transformations, "memory": memory})
            return CandidateProposal(operations=[ProposalOperation(type="rewrite", target="AGENTS.md", reason="semantic fake", expected_clarity_effect="better", expected_finops_effect="same", risk="low")]), {"AGENTS.md": "# Semantic\n"}
    ...
    proposal, rendered = StrategyCandidateGenerator(semantic=FakeSemanticGenerator()).generate(
        snapshot, [], GenerationStrategyName.BALANCED,
        previous_summaries=["C001:rejected:extract"], explored_transformations=["abc"], memory=memory,
    )
