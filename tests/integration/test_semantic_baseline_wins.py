...
def test_baseline_can_win_against_semantically_valid_candidate(tmp_path: Path) -> None:
    ...
    class WorseSemanticGenerator:
        def generate(self, snapshot, invariants, strategy, previous_summaries, explored_transformations, feedback, memory):
            ...
            proposal = CandidateProposal(operations=[ProposalOperation(
                type="rewrite", target="AGENTS.md", reason="semantic expansion",
                expected_clarity_effect="none", expected_finops_effect="worse", risk="low")])
    ...
    result = SearchController(config, runtime, tmp_path / "out", evaluator=PassingEvaluator(),
        semantic_generator=WorseSemanticGenerator()).optimize(baseline, suite, report)
