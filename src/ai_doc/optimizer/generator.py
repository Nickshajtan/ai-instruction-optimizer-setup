...
class CandidateGenerationStrategy(Protocol):
    ...
    def generate(self, snapshot: DocumentationSnapshot, invariants: list[Invariant]) -> tuple[CandidateProposal, dict[str, str]]: ...
...
def _operation(
    ...
    return ProposalOperation(
        type=operation_type, target=target, sources=sources or [], reason=reason,
        expected_clarity_effect=expected_clarity_effect, expected_finops_effect=expected_finops_effect,
        risk=risk, objective=objective or [], trigger=trigger,
    )
...
class ConservativeCandidateGenerator:
    ...
    def generate(self, snapshot: DocumentationSnapshot, invariants: list[Invariant]) -> tuple[CandidateProposal, dict[str, str]]:
        invariant_texts = {invariant.text.strip() for invariant in invariants if invariant.importance == InvariantImportance.CRITICAL}
        ...
        for document in snapshot.documents:
            ...
            if deduped:
                operations.append(_operation("deduplicate", "Repeated bullets or paragraphs were found in the same document.", "Reduces repeated instructions while retaining the first occurrence.", "Lowers repeated always-loaded tokens.", LOW_RISK, target=document.relative_path))
        ...
        if not operations:
            operations.append(_operation("retain", "No low-risk deterministic optimization was found.", "Preserves current structure.", "No material token change.", LOW_RISK))
    ...
    def generate(self, snapshot: DocumentationSnapshot, invariants: list[Invariant]) -> tuple[CandidateProposal, dict[str, str]]:
        ...
        for document in snapshot.documents:
            ...
            if compressed != text:
                ...
                operations.append(_operation("compress", "Instruction file still contains verbose example prose.", "Keeps examples scannable as bullets.", "Reduces always-loaded wording.", MEDIUM_RISK, target=document.relative_path, objective=[OBJECTIVE_CONTEXT_COST]))
        return CandidateProposal(operations=operations), rendered
...
class FeedbackRepairCandidateGenerator:
    ...
    def generate_from_feedback(self, snapshot: DocumentationSnapshot, feedback: OptimizationFeedback) -> tuple[CandidateProposal, dict[str, str]]:
        rendered = {document.relative_path: document.text for document in snapshot.documents}
        operations: list[ProposalOperation] = []
        ...
        for document in snapshot.documents:
            if document.profile != DocumentProfile.INSTRUCTION or STRENGTHEN_FEEDBACK_KEYWORD not in directions:
                continue
            text = rendered[document.relative_path]
            updated = re.sub(r"When detailed ([^.]+) are needed, read", r"When changing, debugging, or validating \1, MUST read", text, flags=re.IGNORECASE)
            if updated != text:
                rendered[document.relative_path] = updated
                operations.append(_operation("strengthen_router", "Parent evaluation showed that the extracted reference router was too weak.", "Makes the trigger explicit without restoring the full section.", "Preserves extraction savings with a short stronger router.", LOW_RISK, target=document.relative_path, objective=[OBJECTIVE_RELIABILITY, OBJECTIVE_CLARITY]))
        if not operations:
            operations.append(_operation("retain", f"Feedback for {feedback.candidate_id} did not identify a deterministic mutation.", "Preserves parent behavior.", "No additional savings.", LOW_RISK))
        return CandidateProposal(operations=operations), rendered
...
class StrategyCandidateGenerator:
    ...
    def generate(
        ...
        if self.semantic is not None and strategy_name != GenerationStrategyName.CONSERVATIVE:
            return self.semantic.generate(snapshot, invariants, strategy_name, summaries, explored, feedback, search_memory)
        if feedback:
...
def _clarity_from_rendered(snapshot: DocumentationSnapshot, rendered: dict[str, str], protected: set[str]) -> tuple[CandidateProposal, dict[str, str]]:
    operations: list[ProposalOperation] = []
    for document in snapshot.documents:
        ...
        for segment in re.split(r"(?<=[.!?])\s+", text):
            if segment.strip() in protected:
                continue
            replacement = re.sub(r"\bUse best practices\b", "Follow the concrete validation, testing, and routing rules documented here", segment, flags=re.IGNORECASE)
        ...
        if updated != text:
            rendered[document.relative_path] = updated
            operations.append(_operation("rewrite", "Ambiguous or inconsistent instruction wording was detected.", "Makes rules more explicit and terminology more consistent.", "May slightly change tokens depending on wording.", LOW_RISK, target=document.relative_path, objective=[OBJECTIVE_CLARITY]))
    if not operations:
        operations.append(_operation("retain", "No deterministic clarity rewrite was available.", "Preserves existing clarity.", "No context change.", LOW_RISK))
    return CandidateProposal(operations=operations), rendered
...
def _extract_large_examples(document: Document, text: str) -> tuple[str, dict[str, str], list[ProposalOperation]]:
    ...
    for section in document.sections:
        if not section.heading or EXAMPLE_HEADING_KEYWORD not in section.heading.title.lower() or section.token_count < EXAMPLES_MIN_TOKENS:
            continue
        ...
        lines[start:end] = [f"{heading_prefix} {section.heading.title}", "", f"When detailed examples are needed, read [{section.heading.title}]({target})."]
        operations.extend([
            _operation("extract", "Large examples are low-frequency detail for an instruction profile.", "Keeps the instruction file focused while preserving examples.", "Moves verbose material out of always-loaded context.", MEDIUM_RISK, target=target, sources=[f"{document.relative_path}#{section.heading.title}"]),
            _operation("add_router", "Extracted material needs a discoverable route.", "Makes on-demand reference discovery explicit.", "Keeps only a short router in always-loaded context.", LOW_RISK, target=document.relative_path, trigger=f"When detailed {section.heading.title.lower()} are needed"),
        ])
        break
